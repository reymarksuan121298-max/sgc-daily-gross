...
export const generateExcelReportWithBarangay = async (data, currentDates, previousDates, regionName = 'MAGUINDANAO') => {
  const workbook = new ExcelJS.Workbook();
  const worksheet = workbook.addWorksheet('Daily Report');
  ...
  const currentDatesTitle = formatDateTitle(currentDates);
  const prevDateRangeStr = formatDateTitle(previousDates).split(' ')[1] || '';
  const prevMonthStr = previousDates.length > 0 ? new Date(previousDates[0]).toLocaleString('default', { month: 'short' }) : '';
  ...
  headerRow2.eachCell((cell, colNumber) => {
    ...
    // Days columns (D-J, i.e., 4-10) are Red
    if (colNumber >= 4 && colNumber <= 10) {
      cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFF0000' } }; // Red
      cell.font = { bold: true, color: { argb: 'FFFFFFFF' } };
    } else if (colNumber === 2 || colNumber === 3) { // Teller and Barangay are cyan
      cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF00FFFF' } }; // Cyan
    }
  });
  ...
  data.forEach((group) => {
    ...
    group.tellers.forEach((teller, index) => {
      ...
      tRow.getCell(2).value = teller.name;
      tRow.getCell(3).value = teller.barangay || teller.address || 'UNKNOWN';
      tRow.getCell(4).value = dayValues[0];
      tRow.getCell(5).value = dayValues[1];
      tRow.getCell(6).value = dayValues[2];
      tRow.getCell(7).value = dayValues[3];
      tRow.getCell(8).value = dayValues[4];
      tRow.getCell(9).value = dayValues[5];
      tRow.getCell(10).value = dayValues[6];
      tRow.getCell(11).value = teller.totalCurr;
      tRow.getCell(12).value = teller.totalPrev;
      
      const analysisCell = tRow.getCell(13);
      analysisCell.value = teller.totalCurr > teller.totalPrev ? 'Increased' : 'Decreased';
      analysisCell.font = { color: { argb: teller.totalCurr > teller.totalPrev ? 'FF0000FF' : 'FFFF0000' }, bold: true };
      
      const diffCell = tRow.getCell(14);
      diffCell.value = teller.difference;
      diffCell.font = { color: { argb: teller.difference < 0 ? 'FFFF0000' : 'FF0000FF' }, bold: true };
      ...
      // Accumulate Unit Totals
      unitTotals.day1 += dayValues[0] || 0;
      unitTotals.day2 += dayValues[1] || 0;
      unitTotals.day3 += dayValues[2] || 0;
      unitTotals.day4 += dayValues[3] || 0;
      unitTotals.day5 += dayValues[4] || 0;
      unitTotals.day6 += dayValues[5] || 0;
      unitTotals.day7 += dayValues[6] || 0;
      unitTotals.totalCurr += teller.totalCurr;
      unitTotals.totalPrev += teller.totalPrev;
      unitTotals.difference += teller.difference;
  ...
  const buffer = await workbook.xlsx.writeBuffer();
  const blob = new Blob([buffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
  saveAs(blob, `${regionName.replace(/ /g, '_')}_Weekly_Gross_Sales_With_Barangay_${currentDatesTitle.replace(/ /g, '_')}.xlsx`);
};
